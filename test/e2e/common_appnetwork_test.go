...
const (
	commonAppnetNamespace = "e2e-common-appnet"
	vipB1                 = "30.0.0.1"
	vipB2                 = "30.0.0.2"
)

var _ = Describe("Common App Network Suite", Ordered, func() {
	...
	Context("Deployment", func() {
		for _, gw := range []string{"gw-b1", "gw-b2"} {
			...
			It(fmt.Sprintf("should have %s Accepted", gw), func() {
				Eventually(func(g Gomega) {
					cmd := exec.Command("kubectl", "get", "gateway", gw, "-n", commonAppnetNamespace,
			...
			It(fmt.Sprintf("should deploy LB Pod for %s", gw), func() {
				Eventually(func(g Gomega) {
					cmd := exec.Command("kubectl", "get", "pods", "-n", commonAppnetNamespace,
		...
		It("should create 2 ENCs (one per target Pod) with 2 gateways each", func() {
			Eventually(func(g Gomega) {
				cmd := exec.Command("kubectl", "get", "enc", "-n", commonAppnetNamespace,
		...
		It("should have 2 target Pods running with sidecar", func() {
			Eventually(func(g Gomega) {
				cmd := exec.Command("kubectl", "get", "pods", "-n", commonAppnetNamespace,
					"-l", "app=target-b", "--field-selector=status.phase=Running",
					"-o", "jsonpath={range .items[*]}{.metadata.name}{\"\\n\"}{end}")
				out, err := utils.Run(cmd)
				g.Expect(err).NotTo(HaveOccurred())
				g.Expect(len(utils.GetNonEmptyLines(out))).To(Equal(2))
