...
Manual testing via Kind cluster with a simulated external BGP peer:

- `docs/router-testing-kind.md`: Step-by-step guide
- `hack/vpn-gateway/`: Docker Compose setup for external BIRD instance
- `test/examples/sample-router-test.yaml`: Complete test manifests (NAD, Gateway, GatewayRouter, Pod with bird-log emptyDir volume)

### Not Tested

- `routing.go` (netlink calls): Requires real network namespace, no unit tests. Direct `netlink` calls with no interface abstraction.
- BIRD process lifecycle: Requires BIRD binary, tested only via integration.
